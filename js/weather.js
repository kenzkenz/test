...
$(function(){
    ...
    $("body").on("change",".weather-select",function() {
        ...
        $.ajax({
            ...
            for (i = 0; i < features.length; i++) {
                ...
                for (j = 0; j < json["kisyoujson"].length; j++) {
                    if (Number(json["kisyoujson"][j]["観測所番号"]) == features[i]["D"]["観測所番号"]) {
                        ...
                        if(maxbb<Number(bb)){
                            maxbb = Number(bb);
                            maxKansokuzyoNo = features[i]["D"]["観測所番号"];
                        }
                        break;
                    }
                }
            ...
            for (i = 0; i < features.length; i++) {
                if (maxKansokuzyoNo == features[i]["D"]["観測所番号"]) {
                    features[i]["D"]["_max"] = 1;
                    break
                }
            }
